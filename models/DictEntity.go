...
const DictTableName string = "Dict"
...
func (table *Dict) Add() error {
	...
	_, err := engine.Table(DictTableName).Insert(&table)
	if err != nil {
		return err
	}
	return nil
...
func (table *Dict) Update() error {
	engine := databases.GetXormEngine()
	table.CreatedBy = ""
	table.CreatedTime = ""
	table.UpdatedTime = carbon.Now().ToDateTimeString()
	table.UpdatedBy = common.GetCurrentUser()
	_, err := engine.Table(DictTableName).Where("id = ?", table.Id).Update(table)
	if err != nil {
		return err
	}
	return nil
...
func (table *Dict) Delete() error {
	engine := databases.GetXormEngine()
	affected, err := engine.Table(DictTableName).Where("id = ?", table.Id).Delete()
	if affected == 0 {
...
func (table *Dict) GetOne(id string) error {
	engine := databases.GetXormEngine()
	_, err := engine.Table(DictTableName).Where("id = ?", id).Desc("id").Get(table)
	if err != nil {
		return err
	}
	return nil
